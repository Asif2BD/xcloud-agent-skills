...
**One release with everything.** Upstream 4.4.0 (the troubleshoot and
performance skills and the shared capability map) was built on 4.3.0, and the
4.3.1–4.3.3 fixes (the ClawHub package, the GET-only REST wrapper and the docs
that match it) were built beside it. 4.4.1 merges the two lines and holds the
new skills to the same read-only REST rule.

### Changed

- **`xcloud:troubleshoot` and `xcloud:performance` on the read-only rule.**
  Their REST calls were already `GET` reads; the intros and scopes now say so,
  and the changes each job may need — WP_DEBUG, rescue, cache purges, a
  PageSpeed scan, a temporary sudo user, server PHP — are written as MCP calls,
  with `confirm: true` where xCloud requires it (argument names checked against
  the live spec).
- **`reference/capability-map.md`** is now where the no-MCP fallback looks:
  the dashboard paths for common changes moved there from `conventions.md`
  (new section **Without an MCP connection**). The `rest` kind now reads
