...
> `https://app.xcloud.host/mcp` → sign in with OAuth. One tool per API
> operation plus `xcloud_agent_search` / `xcloud_docs_search` (or the five-tool
> compact profile), per-action confirmation on destructive operations.
...
It bundles everything — a router `SKILL.md` covering all nine capability areas
(deploy, troubleshoot, performance, servers, sites, WordPress, SSL, billing, account), the wrapper script, and all reference
files. **One upload installs everything.**

> claude.ai treats a zip as a single skill, so all capabilities ship as one
> `xcloud` skill (not nine). That's why there's one zip, not nine.
...
5. Confirm. The skill appears as **`xcloud`** and covers all nine areas — deploy, troubleshoot, performance, servers,
