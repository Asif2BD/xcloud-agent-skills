...
| Read the PHP-FPM error log, the WordPress `debug.log`, the 7G/8G firewall logs, docker-compose, PM2 or OpenClaw logs | ui | **Site → Logs** | `sites.access-logs` (`type=nginx`: the access and error logs), `sites.events`; `sites.wp-debug` only toggles the flag |
...
(`references/shared/conventions.md` → Transports). When a change is asked for and the
