...
- `references/shared/mcp.md` — **prefer the MCP tools when
  ...
  `sites_wordpress_status`); the `$XC` calls below are read-only REST
  fallbacks (`GET` only); changes run on the MCP.
...
Scopes: every REST read needs `read:sites` + `read:servers`. A PageSpeed scan,
a cache purge and server PHP changes run on the MCP (a connection granted write
access); without it, offer to connect (`references/shared/conventions.md` →
Transports). Team
...
| **Disk full, usually backups** | `servers.monitoring` disk near 100%. Everything on the box slows, MySQL first. | Check the site's backup count and the site snapshots on the server (`servers.snapshots`) before blaming the app (the `sites` skill backups) |
...
```text
sites_pagespeed_scan           {"uuid": "<site-uuid>"}   # tell the user a scan is being spent
sites_cache_purge-all          {"uuid": "<site-uuid>"}   # ask first: it empties a warm cache
servers_php-versions_install   {"uuid": "<server-uuid>", "php_version": "8.3"}  # destructive: confirm: true after the user's yes
servers_php-versions_default   {"uuid": "<server-uuid>", "version": "8.3"}  # destructive: confirm: true — moves every site on the default
